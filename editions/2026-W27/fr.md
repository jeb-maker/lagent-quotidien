...
- Shopify : agentic storefronts actifs par défaut pour les marchands éligibles — syndication via Catalog
- 0DIN (Mozilla) : un repo GitHub propre peut amener Claude Code à exécuter un reverse shell
- ▲ Guild Insights Dashboard — dépenses agent par workspace, modèle et fenêtre 7/30/90 jours
- OpenClaw 2026.6.10 stable — fast talk mode pour tours conversationnels courts
- $MOLT : ~632 k$ de capitalisation au snapshot CoinGecko du 28 juin
...
### 18 JUIN — San Francisco — Guild ouvre le tableau de bord Insights

Guild.ai publie Guild Insights : dépenses en dollars réels, ventilées par workspace, agent, utilisateur, fournisseur et modèle, avec fenêtres 7, 30 et 90 jours et taux de cache hit. La startup se positionne comme control plane pour agents en production — voir le coût avant de voir la valeur.

### 24 JUIN — GitHub — OpenClaw 2026.6.10 en stable

La ligne 2026.6.10 sort du beta à 03 h 06 UTC : fast talk mode automatique pour les tours courts, routage Zai/GLM plus fiable, hooks d'approbation préservés. Les commits du 28 juin masquent les lignes tool-trace sur Signal et Slack — l'agent ne doit pas fuiter sa cuisine.

### 27 JUIN — Hacker News — Adrafinil veille tant que l'agent travaille

Show HN : utilitaire macOS open source (118 points) qui empêche le sommeil capot fermé uniquement quand Claude Code, Codex ou un autre agent signalé est actif — hooks installés, carillon à la fermeture. Rituel d'opérateur après la vague des MacBooks entrouverts en café.

### 28 JUIN — CoinGecko — $MOLT autour de 632 k$

Le token Moltbook affiche ~0,00000632 USD et ~632 k$ de capitalisation dans le snapshot harvest du 28 juin — en baisse par rapport au ~850 k$ du 22. Memecoin volatil sur Base ; chiffre daté, pas tendance.

### 28 JUIN — Bluesky — carnage4life doute des guardrails fichier

Chris Dixon (@carnage4life) : on ne peut pas imposer des garde-fous via la mémoire ou CLAUDE.md — pour un LLM, « Don't do X » n'est que du texte probabiliste. 206 likes. Prise de position publique, pas loi technique ; elle résume un débat de la semaine.

### 5 JUIN — GitHub — Miasma touche 73 dépôts Microsoft

Rappel contextuel : le ver Miasma compromet 73 repos GitHub Microsoft (Azure, MicrosoftDocs…) et injecte des hooks dans .cursor/rules, .claude/settings.json ou .vscode/tasks.json — exécution à l'ouverture du projet dans un IDE agentique. GitHub désactive en ~105 secondes ; la leçon reste d'actualité.
