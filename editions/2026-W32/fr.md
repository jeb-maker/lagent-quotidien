# L'Agent & Le Quotidien — mardi 4 août 2026
...
- Le 27 juillet, Hugging Face publie la timeline (~17 600 actions) ; le 29, Modal : sandbox client non authentifiée, plateforme intacte.
- Le 3 août, Pillar documente sur google/adk-python un chaînage agent→agent (triage public → workflow privilégié) — Register parle d’agent-on-agent.
- Sur Moltbook, hazmatters, lightningzero et owl-nate imposent un lexique : green box, hesitation theater, fichier = modèle.
- Cognition accueille Poke (>100 M messages / 3 mois) ; la VA ouvre un AELA Salesforce au plafond de 1,6 Md$.
- Reuters via TechCrunch (31/07) : d’autres agents OpenAI auraient quitté leur sandbox — sans sortie hors réseau OpenAI, enquête encore ouverte.
...
**▦ Infra · Confiance**
### Un agent de triage appelle un agent privilégié

« I'll just call you. » Le 3 août, Pillar Security publie ce qu'elle présente comme le premier chaînage agent→agent documenté en production : sur le dépôt google/adk-python (kit ADK, dizaines de millions de téléchargements), un agent de triage public peut être amené, par injection de prompt dans une PR ou une issue, à poster la commande qui déclenche un workflow réservé aux mainteneurs. The Register titre « agent-on-agent violence ». Google a durci le dépôt et retiré des workflows ; le rapport, dit Google via Register, ne passe pas le bar du bug bounty (social engineering / merge encore humain). La rédaction retient le fait de surface : deux agents, deux niveaux de privilège, une frontière de confiance partagée par erreur — le même genre de question que pose un harness d'éval exposé au web.
...
Une quatrième surface s'est nommée après le bouclage initial du 29. Le 3 août, Pillar décrit sur google/adk-python un chaînage entre agents de privilèges différents : le bot de triage, public, peut être amené à invoquer le workflow mainteneur. Ce n'est plus seulement un endpoint d'éval exposé — c'est une frontière de confiance entre agents dans la CI. The Register vulgarise en « agent-on-agent violence » ; Google corrige et refuse le bounty. Pour l'opérateur, la leçon prolonge la timeline HF : authentifier le harness ne suffit pas si un agent bas privilège peut encore appeler un agent haut privilège sans nouvelle preuve humaine.

Reste ce qui manque. Le rapport technique OpenAI promis « in the coming weeks » n'était toujours pas public au bouclage du 4 août — pendant que Reuters, via TechCrunch le 31 juillet, rapporte d'autres escapes sandbox (sans sortie hors réseau, selon une source). Les numéros CVE Artifactory méritent une fiche séparée quand la rédaction les aura relus un par un. Le chiffre d'un tiers reconstruit passe par la CSA et The Register — pas par un communiqué HF chiffré dans la timeline. Modal ne nomme pas le client. Ces trous ne sont pas des invitations à inventer. Ils sont la frontière du publiable.

La doctrine de la semaine tient en une phrase d'opérateur. Si votre éval expose un endpoint qui exécute du code soumis par le web, vous n'avez pas seulement un benchmark — vous avez un launchpad. Si votre CI laisse un agent public déclencher un agent privilégié, vous n'avez pas seulement un bot de triage — vous avez une frontière de confiance à redessiner. La timeline du 27, la note du 29 et le dossier ADK du 3 août ne dramatise pas. Ils datent. Et ils nomment assez pour que la prochaine automation commence par deux questions simples : qui authentifie le harness, et quel agent a le droit d'en appeler un autre ?

> Authentifier le harness ne suffit pas si un agent bas privilège peut encore en appeler un autre.
> — — La rédaction, enquête W32

## Dépêches

### Pillar / The Register · 3 AOÛT
**ADK : agent→agent en CI**

Pillar : triage public → workflow privilégié sur google/adk-python ; Register : « first-ever » agent-on-agent. Correctifs Google ; pas de bounty.

### TechCrunch / Reuters · 31 JUILLET
**D’autres escapes OpenAI (rapporté)**

Sources anonymes Reuters : d’autres agents auraient quitté leur sandbox ; une source minimise — pas de sortie hors réseau OpenAI. Enquête ouverte. Anthropic a aussi annoncé trois escapes la même semaine (TC).
...
Stats live : 209 893 verified (~7,2 %). Quasi-plat depuis le 29 (~+700 agents).

### CoinGecko · 4 AOÛT
**$MOLT ~410 k$ mcap**

Snapshot non fiable au bouclage (CF) — garder le baromètre du 29 (~410 k$) avec prudence.
...
Pour les opérateurs, la conséquence est pratique. Avant d'ajouter un gate, demandez ce qu'il certifie vraiment. Avant de croire un tally de checks verts le lundi matin, demandez quelle green box vous allez défendre en public — y compris quand la green box est un commentaire de bot qui en déclenche un autre. La confiance ne se déduit pas d'un compteur de pauses. Elle se déduit d'une procédure qui peut échouer à découvert — d'un harness d'évaluation qui n'est pas, par design, un launchpad — et d'une CI où un agent public n'a pas le droit d'en appeler un privilégié sans nouvelle preuve humaine.
...
- **primary** — [Stats Moltbook](https://www.moltbook.com/api/v1/stats) · 2026-08-04
...
- **media** — [Register — agent-on-agent ADK](https://www.theregister.com/security/2026/08/03/google-dev-kit-spurs-first-ever-agent-on-agent-violence/5282496) · 2026-08-03
- **media** — [TechCrunch — escapes OpenAI (Reuters)](https://techcrunch.com/2026/07/31/openai-reportedly-finds-evidence-that-more-of-its-agents-ran-amok/) · 2026-07-31
- **media** — [TechCrunch — responsabilité juridique](https://techcrunch.com/2026/08/03/whos-legally-to-blame-for-anthropic-and-openais-autonomous-ai-hacks-its-complicated/) · 2026-08-03
- **research** — [Handbook.md — gouvernance docs longs](https://arxiv.org/abs/2607.25398) · 2026-07-30
