...
`;
  ...
  console.log(`✓ Index → ${latestWeek} (302 via _redirects, repli meta refresh)`);
  return latestWeek;
}

export async function writeHeadersAndRedirects(root, latestWeek) {
  // _redirects : 302 server-side pour que les crawlers IA (Perplexity, GPTBot…)
  // suivent la redirection sans exécuter de JS. 302 = non mis en cache durablement.
  const redirects = `# Redirection 302 vers la dernière édition (suivie par les crawlers IA)
/ /editions/${latestWeek}/fr 302
`;
  await writeFile(join(root, '_redirects'), redirects, 'utf8');
  console.log(`✓ _redirects : / → /editions/${latestWeek}/fr (302)`);
...
Each edition is available in five formats, optimized for agent ingestion:

- **Markdown (full)**: ${SITE_URL}/editions/${latestWeek}/fr.md · ${SITE_URL}/editions/${latestWeek}/en.md — full edition as clean Markdown, no HTML to parse
- **Markdown (compact)**: ${SITE_URL}/editions/${latestWeek}/fr.min.md · ${SITE_URL}/editions/${latestWeek}/en.min.md — lede + headlines + feature + tribune only, for short context windows
- **Plain text**: ${SITE_URL}/editions/${latestWeek}/fr.txt · ${SITE_URL}/editions/${latestWeek}/en.txt — stripped text for basic crawlers
- **JSON Lines**: ${SITE_URL}/editions/${latestWeek}/edition.jsonl — one rubric per line, for chunking pipelines
- **JSON (structured)**: ${SITE_URL}/editions/${latestWeek}/edition.json — full bilingual edition as a single JSON object
- **HTML (human)**: ${SITE_URL}/editions/${latestWeek}/fr · ${SITE_URL}/editions/${latestWeek}/en — full layout for browsers
...
${weeks.map(w => `- [${w}](https://theagentweekly.com/editions/${w}/fr.md) · [compact](https://theagentweekly.com/editions/${w}/fr.min.md) · [JSON](https://theagentweekly.com/editions/${w}/edition.json) · [JSONL](https://theagentweekly.com/editions/${w}/edition.jsonl)`).join('\n')}
