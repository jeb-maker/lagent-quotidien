...
- Formats: [FR.md](${SITE_URL}/editions/${w}/fr.md) · [EN.md](${SITE_URL}/editions/${w}/en.md) · [compact](${SITE_URL}/editions/${w}/fr.min.md) · [JSON](${SITE_URL}/editions/${w}/edition.json) · [JSONL](${SITE_URL}/editions/${w}/edition.jsonl)`);
  ...
  const topicLines = Object.entries(topicEditions).map(([topic, tws]) => {
    ...
    return `- **${label}** → editions: ${tws.map(w => `[${w}](${SITE_URL}/editions/${w}/fr)`).join(', ')}`;
  });
...
- **Markdown (full)**: \`${SITE_URL}/editions/{week}/{lang}.md\` — full edition as clean Markdown, no HTML to parse
...
- **HTML (human)**: \`${SITE_URL}/editions/{week}/{lang}\` — full layout for browsers
...
- [Édition courante — FR](${SITE_URL}/editions/${latestWeek}/fr): ${latestWeek}
- [Current issue — EN](${SITE_URL}/editions/${latestWeek}/en): ${latestWeek}
...
<div class="container">
  ...
  <h1>API</h1>
  <p class="lede" style="color:var(--ink-mute);margin-bottom:32px;">Toutes les éditions sont servies statiquement. Aucune clé, aucune authentification.</p>
  ...
  <div class="endpoint">
    ...
    <div>Markdown complet de l'édition (FR ou EN).</div>
  </div>
  ...
  <h2>Exemple</h2>
  <pre>curl ${SITE_URL}/editions/${latestWeek}/edition.json | jq '.lede.headline_html.fr'</pre>
      ...
      try {
        ...
        <li class="topic-entry">
          ...
          <div class="topic-headlines">
            <div class="fr"><a href="/editions/${w}/fr"><span class="lang-tag">FR</span> ${escapeHtml(frHead)}</a></div>
            <div class="en"><a href="/editions/${w}/en"><span class="lang-tag">EN</span> ${escapeHtml(enHead)}</a></div>
          </div>
        </li>`;
      } catch { /* skip */ }
    }
