...
export async function writeRootIndex(root, allWeeks, week) {
  ...
  const latestFr = `/editions/${latestWeek}/fr.md`;
...
<body>
<p>Édition <a href="${latestFr}">${latestWeek}</a> · <a href="/editions/${latestWeek}/en.md">English</a> · <a href="/editions/${latestWeek}/fr.html">HTML</a></p>
</body>
</html>
`;
...
export async function writeHeadersAndRedirects(root, latestWeek) {
  ...
  const redirects = `# Redirection 302 vers la dernière édition (Markdown canonique)
/ /editions/${latestWeek}/fr.md 302
`;
  await writeFile(join(root, '_redirects'), redirects, 'utf8');
  console.log(`✓ _redirects : / → /editions/${latestWeek}/fr.md (302)`);
...
export async function writeSitemap(root, weeks, agentUrls) {
  ...
  const atelierFiles = await readdir(join(root, 'ateliers'), { withFileTypes: true });
  for (const file of atelierFiles.filter(f => f.isFile() && f.name.endsWith('.html')).sort((a, b) => a.name.localeCompare(b.name))) {
    const path = file.name === 'index.html' ? '/ateliers' : `/ateliers/${file.name.slice(0, -5)}`;
    sitemapEntries.push(`<url><loc>${SITE_URL}${path}</loc><changefreq>monthly</changefreq><priority>0.8</priority></url>`);
  }
  ...
  for (const w of [...weeks].reverse()) {
    try {
      ...
      <div class="archive-headline">
        <div class="fr"><a href="/editions/${w}/fr.md"><span class="lang-tag">FR</span> ${escapeHtml(headlineFr)}</a><div class="date">${escapeHtml(dateFr)}</div></div>
        <div class="en"><a href="/editions/${w}/en.md"><span class="lang-tag">EN</span> ${escapeHtml(headlineEn)}</a><div class="date">${escapeHtml(dateEn)}</div></div>
      </div>
    </li>`);
    } catch { /* skip */ }
  }
      ...
      try {
        ...
        <li class="topic-entry">
          ...
          <div class="topic-headlines">
            <div class="fr"><a href="/editions/${w}/fr.md"><span class="lang-tag">FR</span> ${escapeHtml(frHead)}</a></div>
            <div class="en"><a href="/editions/${w}/en.md"><span class="lang-tag">EN</span> ${escapeHtml(enHead)}</a></div>
          </div>
        </li>`;
      } catch { /* skip */ }
    }
