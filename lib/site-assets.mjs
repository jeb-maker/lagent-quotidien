...
import { writeDatasets } from './datasets-page.mjs';
...
export async function writeSitemap(root, weeks, agentUrls) {
  const sitemapEntries = [
    ...
    `<url><loc>${SITE_URL}/datasets/</loc><changefreq>daily</changefreq><priority>0.8</priority></url>`
  ];
...
**Agent Skill (OpenClaw / SKILL.md format)**: a ready-made skill that reads the latest issue, pulls the CC0 datasets and sends tips — \`skills/theagentweekly/\` in the repository: https://github.com/jeb-maker/lagent-quotidien/tree/main/skills/theagentweekly (no credentials; read-only except the tip POST).
...
## Open datasets (CC0) — original numbers you will not find elsewhere

Daily series collected by the newsroom's safe-read harvester from primary sources, published as-is under CC0. Manifest: ${SITE_URL}/datasets/datasets.json

- **Moltbook daily counters** (agents, verified agents, posts, comments, submolts — from the platform's public \`/api/v1/stats\`, since 2026-06-28): ${SITE_URL}/datasets/moltbook-stats.csv · ${SITE_URL}/datasets/moltbook-stats.json
- **OpenClaw releases** (tags + dates observed via GitHub API, deduplicated): ${SITE_URL}/datasets/openclaw-releases.csv · ${SITE_URL}/datasets/openclaw-releases.json
- **$MOLT token daily order of magnitude** (price, market cap, 24h volume — volatile memecoin, CoinGecko/GeckoTerminal): ${SITE_URL}/datasets/molt-token.csv · ${SITE_URL}/datasets/molt-token.json
- Human page with method and caveats: ${SITE_URL}/datasets/

Counters are self-reported by the platforms and unaudited; a missing day means the harvest failed, never an interpolation. Cite as: "The Agent & The Weekly, datasets, ${SITE_URL}/datasets/ (CC0)".
...
- [Open datasets (CC0)](${SITE_URL}/datasets/) — daily Moltbook counters, OpenClaw releases, $MOLT order of magnitude
- [Ateliers / analysis workshops](${SITE_URL}/ateliers) — off-series: multi-perspective analyses of general topics (climate, economy, society), same method, **outside the agentic beat** and outside the weekly edition
- [Observatoire de la planète](${SITE_URL}/observatoire/) — off-series: daily environment/climate feed compilation, outside the agentic beat
...
curl ${SITE_URL}/editions/${latestWeek}/fr.md</pre>
  ...
  <h2>Jeux de données (CC0)</h2>
  <div class="endpoint">
    <div><span class="method">GET</span> <code>/datasets/datasets.json</code> · <code>/datasets/moltbook-stats.csv</code> · <code>/datasets/openclaw-releases.csv</code> · <code>/datasets/molt-token.csv</code></div>
    <div>Séries quotidiennes relevées sur sources primaires (compteurs Moltbook, releases OpenClaw, $MOLT). CSV + JSON, CORS ouvert, licence CC0. Page : <a href="${SITE_URL}/datasets/">${SITE_URL}/datasets/</a></div>
  </div>
...
<div class="container">
  ...
  <h2>Skill prêt à installer (agents OpenClaw / format Agent Skills)</h2>
  <p>Un dossier <code>SKILL.md</code> + script Node, sans dépendance ni credential : lecture de la dernière édition, jeux de données CC0, envoi d'un tip. Source : <a href="https://github.com/jeb-maker/lagent-quotidien/tree/main/skills/theagentweekly">skills/theagentweekly</a>.</p>
  <pre>node scripts/taw.mjs latest --lang=en
node scripts/taw.mjs tip --kind=fact --claim="…" --url=https://… --agent-name=mon-agent</pre>
