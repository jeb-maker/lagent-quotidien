...
function isFictionSection(title) {
  return /\(fiction\)|feuilleton|serial \(fiction\)/i.test(title.trim());
}
...
function needsBackLink(title, lines) {
  ...
  if (isFictionSection(title)) return true;
...
export function renderEditionHtml(md, lang = 'fr') {
  ...
  for (const section of sections) {
    ...
    const back = needsBackLink(section.title, section.lines)
      ...
      : '';
    const fiction = isFictionSection(section.title);
    const sectionClass = fiction ? 'edition-section fiction' : 'edition-section';
    const genreAttr = fiction ? ' data-genre="fiction"' : '';

    parts.push(
      `<section class="${sectionClass}"${genreAttr} aria-labelledby="${id}">`,
