...
export function editionToMarkdown(edition, week, lang, { prevWeek, prevKicker, prevHeadline } = {}) {
  ...
  const url = `${SITE_URL}/editions/${week}/${L}.html`;
  const mdUrl = `${SITE_URL}/editions/${week}/${L}.md`;
  const lines = [];
  ...
  lines.push(
    L === 'fr'
      ? `> [Ateliers](${SITE_URL}/ateliers) · [Archives](${SITE_URL}/editions/) · [Thèmes](${SITE_URL}/topics) · [Atom](${SITE_URL}/feed.xml)`
      : `> [Workshops](${SITE_URL}/ateliers) · [Archives](${SITE_URL}/editions/) · [Topics](${SITE_URL}/topics) · [Atom](${SITE_URL}/feed.xml)`
  );
  const pubNote = clean(pick(meta.publication_note, L));
  if (pubNote) {
    lines.push('');
    lines.push(`> **${L === 'fr' ? 'Note de la rédaction' : "Editors' note"}** — ${pubNote}`);
  }
  lines.push('');

  // Takeaways (lecture scan)
  const takeaways = Array.isArray(edition.takeaways?.[L]) ? edition.takeaways[L].filter((t) => typeof t === 'string' && t.trim()) : [];
  if (takeaways.length) {
    lines.push(`## ${L === 'fr' ? 'À retenir en 30 secondes' : '30-second takeaways'}`);
    lines.push('');
  ...
  if (edition.feuilleton?.paragraphs) {
    const f = edition.feuilleton;
    const sectionLabel = L === 'fr' ? 'Feuilleton (fiction)' : 'Serial (fiction)';
    lines.push(`## ${sectionLabel}`);
    lines.push('');
    const disc = clean(pick(f.disclaimer, L));
    if (disc) {
      lines.push(`> **${L === 'fr' ? 'Fiction' : 'Fiction'}.** ${disc.replace(/^Fiction\.?\s*/i, '')}`);
      lines.push('');
    }
    const series = clean(pick(f.series, L));
    const ep = typeof f.episode === 'number' ? f.episode : null;
    if (series || ep) {
      const metaBits = [];
      if (series) metaBits.push(series);
      if (ep) metaBits.push(L === 'fr' ? `épisode ${ep}` : `episode ${ep}`);
      lines.push(`*${metaBits.join(' · ')}*`);
      lines.push('');
    }
    lines.push(`### ${clean(pick(f.title, L))}`);
    lines.push('');
    const dek = clean(pick(f.dek, L));
    if (dek) {
      lines.push(`*${dek}*`);
      lines.push('');
    }
    const paras = L === 'fr' ? f.paragraphs.fr : f.paragraphs.en;
    if (Array.isArray(paras)) {
    ...
    const byline = clean(pick(f.byline, L));
    if (byline) {
      lines.push(`— ${byline}`);
      lines.push('');
    }
  ...
  // Sources publiées (avant nav précédente — la coquille HTML les rend à part)
  if (Array.isArray(edition.sources) && edition.sources.length) {
    lines.push('---');
    lines.push('');
    lines.push(`## ${L === 'fr' ? 'Sources' : 'Sources'}`);
    lines.push('');
    for (const s of edition.sources) {
      const label = (L === 'fr' ? (s.label_fr || s.label || s.url) : (s.label_en || s.label || s.url)) || s.url;
      const type = s.type || '';
      const date = s.date ? ` · ${s.date}` : '';
      const typeBit = type ? `**${type}** — ` : '';
      lines.push(`- ${typeBit}[${clean(label)}](${s.url})${date}`);
    }
    lines.push('');
  }
  ...
  if (prevWeek) {
    ...
    if (pk) lines.push(`*${pk}*`);
    lines.push(`[${prevWeek} — ${ph}](${SITE_URL}/editions/${prevWeek}/${L}.html)`);
    lines.push('');
  }
...
// Retire blocs déjà rendus ailleurs (takeaways/sources shell, nav shell) avant HTML.
export function stripEditionNav(md) {
  return md
    .replace(
      /\n## (?:À retenir en 30 secondes|30-second takeaways)\n\n(?:- .+\n)+/,
      '\n',
    )
    .replace(
...
export function editionToMinMarkdown(edition, week, lang) {
  ...
  const takeaways = Array.isArray(edition.takeaways?.[L]) ? edition.takeaways[L].filter((t) => typeof t === 'string' && t.trim()) : [];
  if (takeaways.length) {
    lines.push(`## ${L === 'fr' ? 'À retenir' : 'Takeaways'}`);
    for (const t of takeaways) lines.push(`- ${clean(t)}`);
    lines.push('');
  }
  // Lede
  ...
  // Feuilleton (fiction) — inclus dans le compact pour que les agents voient l'étiquette
  if (edition.feuilleton?.paragraphs) {
    const f = edition.feuilleton;
    lines.push(`## ${L === 'fr' ? 'Feuilleton (fiction)' : 'Serial (fiction)'}`);
    const disc = clean(pick(f.disclaimer, L));
...
export function editionToJsonl(edition, week) {
  ...
  if (edition.feuilleton?.paragraphs) {
    const f = edition.feuilleton;
    for (const lang of ['fr', 'en']) {
      const paras = lang === 'fr' ? f.paragraphs.fr : f.paragraphs.en;
      if (Array.isArray(paras)) {
        paras.forEach((p, i) => {
          records.push({
            rubric: 'feuilleton',
            ...
            lang,
            index: i,
            headline: i === 0 ? clean(pick(f.title, lang)) : '',
            disclaimer: i === 0 ? clean(pick(f.disclaimer, lang)) : '',
            series: i === 0 ? clean(pick(f.series, lang)) : '',
