...
// Comparaison timing-safe : on hashe les deux côtés (longueurs égales garanties)
// puis XOR byte à byte, sans court-circuit.
...
async function storeTip(env, record) {
  ...
  // Clé datée : le harvest filtre par préfixe, pas d'index à maintenir
  // (l'ancien index idx:<date> était en read-modify-write non atomique).
  const key = `tip:${dayKey()}:${record.id}`;
  await env.TIPS_KV.put(key, JSON.stringify(record), { expirationTtl: TIP_TTL_SECONDS });
  return record.id;
}
...
async function listTips(env, sinceDay) {
  ...
  let cursor;
  do {
    const page = await env.TIPS_KV.list({ prefix: 'tip:', cursor });
    for (const { name } of page.keys) {
      // name = tip:YYYY-MM-DD:uuid — filtre lexicographique sur la date
      if (name.slice(4, 14) < sinceDay) continue;
      const raw = await env.TIPS_KV.get(name);
...
async function handleHarvest(req, env) {
  ...
  if (!token || !(await timingSafeEqual(auth, `Bearer ${token}`))) {
  ...
  // Défaut 7 jours ; clamp à 60 jours max (aligné sur le TTL des tips)
  const minDay = dayKey(new Date(Date.now() - MAX_SINCE_DAYS * 864e5));
  let sinceDay = since || dayKey(new Date(Date.now() - DEFAULT_SINCE_DAYS * 864e5));
  if (sinceDay < minDay) sinceDay = minDay;
  const tips = await listTips(env, sinceDay);
